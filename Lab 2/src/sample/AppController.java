...
public class AppController implements Initializable {
    ...
    public void labelOnClick() {
        ...
        if (pointChosed) {
            ...
            graph.sortPoints();
            graph.setWeights();
            ArrayList<ArrayList<GraphNode>> a =  graph.buildChains();
            ArrayList<Pair<List<GraphNode>, Integer>> chains = graph.findChainsContainingPoint(a, point);
            if (chains == null) {
                label.setText("Точка поза графом");
            } else {
                context.setStroke(Color.BLUE);

                for (Pair<List<GraphNode>, Integer> chain: chains) {
                    for (int i = 0; i < chain.getKey().size() - 1; i++) {
                        context.strokeLine(chain.getKey().get(i).position.x, chain.getKey().get(i).position.y, chain.getKey().get(i +  1).position.x, chain.getKey().get(i + 1).position.y);
                    }
                }
    ...
    public void canvasClick(MouseEvent event) {
        ...
        if (firstPointChosen != -1) {
            ...
            Point firstPoint = graph.getPoint(firstPointChosen);
            Point secondPoint = graph.getPoint(secondPointIndex);
